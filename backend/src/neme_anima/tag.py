...
def tag_crops(
    *,
    ...
    # metadata.jsonl を更新 (既存エントリは tags/rating を上書き、なければ追記)
    if metadata_rows:
        ...
        idx = {r["filename"]: i for i, r in enumerate(existing)}
        for row in metadata_rows:
            fn = row["filename"]
            if fn in idx:
                existing[idx[fn]]["tags"] = row["tags"]
                existing[idx[fn]]["rating"] = row.get("rating", "general")
            else:
                existing.append(row)
